...
Agents use a pre-configured API key:
...
`ApiKeyTokenProvider` implements `ITokenProvider` and returns the configured API key directly.
...
├── ApiKeyTokenProvider    # Returns configured API key
├── Auth0TokenProvider     # M2M Auth0 (for future use)
└── DeviceFlowAuthenticator # Interactive login flow
...
| `STUDYWISE_API_BASE_URL` | Studywise API base URL (default: `https://api.studywise.se`) |
